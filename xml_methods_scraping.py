...
url = "file:///C:/Users/ryosuke-ku/Desktop/TCS/NICAD/projects/" + ProjectName + "_functions-blind-clones/" + ProjectName + "_functions-blind-clones-0.30-classes-withsource.html"
...
# filePath = soup.find('table')
# print(filePath)

# for product in filePath.find_all(['tr', 'td']):
# 	print(product)
...
product_div = soup.find('table')

key = []
value = []

for srccode in product_div.find_all(['pre']):
	value.append(srccode.text)
	print(srccode.text)

for product in product_div.find_all(['td']):
	product.find('pre').decompose()
	key.append(product.text)
	print(product.text)
