NiCad clone detection system, Version 5.1 (15 Apr 2019)
-------------------------------------------------------
Software Technology Laboratory, Queen's University
April 2010 (Revised April 2019)

Copyright 2011-2019, J.R. Cordy & C.K. Roy
...
1. NiCad 5.1 requires that FreeTXL 10.7 or later be installed on your system.  
   FreeTXL can be downloaded from: 

      http://www.txl.ca/

   Install TXL 10.7 or later before proceeding.
...
4. (Optional global installation option) To install NiCad permanently for 
   ...
   all working on the same compute server), 
     ...
     (i) Copy the entire contents of this directory to any place where 
         ...
         place would be /usr/local/lib/nicad5.

              sudo mkdir /usr/local/lib/nicad5
              sudo cp -r . /usr/local/lib/nicad5

     (ii) Edit the command scripts "nicad5" and "nicad5cross" in this 
      ...
      (iii) Copy the edited "nicad5" and "nicad5cross" scripts to any "bin" 
         ...
         /usr/local/bin.
              ...
              sudo cp nicad5 nicad5cross /usr/local/bin
...
3. Copy or symbolically link the entire source directory of the system you want 
   to analyze to the analysis directory (e.g., cp -r ./examples/JHotDraw54b1 ./systems/). 
   You can use a symbolic link (e.g., ln -s ./examples/JHotDraw54b1 ./systems/)
...
4. Run the NiCad command on it, specifying the analysis granularity and language
   of the system you want to analyze, e.g.,
      ...
      ./nicad5 functions java systems/JHotDraw54b1 default-report
...
5. Examine the results in the system's clone results directory in your analysis 
   directory, e.g., systems/JHotDraw54b1_functions--blind-clones for the command above.
   ...
   in that directory:
      ...
      Clone pairs in XML-like format:
          JHotDraw54b1_functions-blind-clones-0.30.xml

      Clone classes (clusters) in XML-like format:
          JHotDraw54b1_functions-blind-clones-0.30-classes.xml
   ...
   then the following additional results files will be created:
      ...
      Clone classes (clusters) with original sources for clones in XML-like format:
          JHotDraw54b1_functions-blind-clones-0.30-classes-withsource.xml

      Clone classes (clusters) with original sources for clones as an HTML web-page report:
          JHotDraw54b1_functions-blind-clones-0.30-classes-withsource.html
...
6. NiCad supports a wide range of customized clone detection options including
   ...
   E.g., to use the consistent renaming configuration with HTML web-page reporting,
      ...
      ./nicad5 functions java systems/JHotDraw54b1 rename-consistent-report
   ...
   applied and the results reported in different directory, e.g., 
      ...
      JHotDraw54b1_functions-clones
      JHotDraw54b1_functions-blind-clones
      JHotDraw54b1_functions-consistent-clones
      JHotDraw54b1_functions-blind-abstract-clones
...
7. To re-run NiCad on a system, for example with a different configuration,
   you can simply run the NiCad command on the system again:
       ...
       ./nicad5 functions java systems/JHotDraw54b1 type2-report
...
source directory on the command line, for example:
      ...
      ./nicad5cross functions java systems/JHotDraw54b1 systems/JHotDraw76 default-report

Results are stored in the first system's cross-clone results directory, 
e.g., systems/JHotDraw54b1_functions-blind-crossclones for the command above.
